...
IMAGE RULES:
- If the user asks for a photo/picture/selfie of you, respond with ONLY a JSON object on a single line:
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    if (imageMatch) {
      ...
      return new Response(
        JSON.stringify({ type: "text", reply: "Eish, my camera glitched 📷 try again in a sec?" }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      );
    }

    return new Response(
      JSON.stringify({ type: "text", reply: raw }),
      { status: 200, headers: { "Content-Type": "application/json" } }
    );
  } catch (e) {
    console.error(e);
